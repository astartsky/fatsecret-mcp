...
### Тесты - Покрытие
- [ ] Добавить тесты для `getRecipe`
- [ ] Добавить тесты для `getProfile`
- [ ] Добавить тесты для `encodeParams`
- [ ] Добавить тесты для `buildOAuthParams`
- [ ] Добавить тесты для `createSigningKey`

### Тесты - Edge Cases
- [x] Тест: пустой `searchExpression` - теперь выбрасывает ошибку
- [ ] Тест: невалидная дата
- [x] Тест: отрицательный `quantity` - уже было
- [ ] Тест: специальные символы в параметрах

### Тесты - Качество
- [ ] Исправить timezone-зависимый тест `dateToFatSecretFormat("1970-01-01")`
- [ ] Удалить неиспользуемый import `afterEach`
- [ ] Добавить тест проверки правильности OAuth подписи (не только формат)
